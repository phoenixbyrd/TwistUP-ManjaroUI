...
getchangelog() {
  ...
  messagetxt="$(wget -qO- https://twisteros.com/Patches/TwisterLite/messageLite.txt)"
...
patch2url() {
  ...
  URL="$(wget -qO- https://twisteros.com/Patches/TwisterLite/URLsLite | grep "$1" | awk '{print $2}')"
  if [ $? != 0 ] || [ -z "$URL" ];then
...
patchlist="$(wget -qO- https://twisteros.com/Patches/TwisterLite/URLsLite)"
