...
getchangelog() {
  ...
  messagetxt="$(wget -qO- https://twisteros.com/Patches/TwisterManjaroUI/messageManjaroUI.txt)"
...
patch2url() {
  ...
  URL="$(wget -qO- https://twisteros.com/Patches/TwisterManjaroUI/URLsManjaroUI | grep "$1" | awk '{print $2}')"
  if [ $? != 0 ] || [ -z "$URL" ];then
...
if [[ "$runmode" == gui* ]] && [ ! -f '/usr/bin/yad' ];then
  error "YAD is required but not installed. Please run 'pacman -Syu yad' in a terminal."
...
patchlist="$(wget -qO- https://twisteros.com/Patches/TwisterManjaroUI/URLsManjaroUI)"
